...
public sealed record CitySearchQuery
{
    ...
    private CitySearchQuery() { }

    public static CitySearchQuery For(string input, Language? lang = null)
    {
        if (string.IsNullOrWhiteSpace(input))
            throw new ArgumentException("Location is required.", nameof(input));

        return new CitySearchQuery { Query = input, Lang = lang };
    }
}
